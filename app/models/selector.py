...
def get_model(device, generative, action_converter, token_converter, tag_converter, nt_converter, config):
    ...
    """
    assert config.dropout_type in ['normal', 'variational']
    if config.type == 'rnng' or config.type == 'parallel_rnng':
        ...
        representation = get_representation(device, config.size.rnn, config.rnn.hidden_size, config.dropout_type, config.representation)
        ...
        if config.type == 'rnng':
            ...
            action_history = History(device, get_rnn(device, config.size.action, config.dropout_type, config.rnn))
            token_buffer = Buffer(device, get_rnn(device, config.size.rnn, config.dropout_type, config.rnn))
            stack = Stack(get_rnn(device, config.size.rnn, config.dropout_type, config.rnn))
            ...
            action_history = HistoryLSTM(device, get_rnn(device, config.size.action, config.dropout_type, config.rnn))
            stack = StackLSTM(device, config.size.rnn, *rnn_args, config.rnn.weight_drop, config.dropout_type, config.rnn.type == 'layer_norm_lstm')
            ...
            if generative:
                ...
                token_buffer = OutputBufferLSTM(device, get_rnn(device, config.size.rnn, config.dropout_type, config.rnn))
                ...
                pos_embedding = get_embedding(tag_converter.count(), config.size.pos, config.pos_emb_drop, config.embedding)
                token_buffer = InputBufferLSTM(device, get_rnn(device, config.size.rnn, config.dropout_type, config.rnn))
                structures[1] = token_buffer
        ...
        rnn = get_rnn(device, config.size.rnn, config.dropout_type, config.rnn)
        ...
        action_embedding_size = config.size.rnn
        encoder_rnn = get_rnn(device, config.size.rnn, config.dropout_type, config.rnn)
        encoder_output_size = encoder_rnn.get_output_size()
        decoder_input_size = action_embedding_size + encoder_output_size
        decoder_rnn = get_rnn(device, decoder_input_size, config.dropout_type, config.rnn)
        action_count = action_converter.count()
