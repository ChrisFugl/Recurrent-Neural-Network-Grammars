def get_iterator(device, action_converter, token_converter, tag_converter, tokens, actions_strings, tags, config):
    """
    ...
    if config.type == 'ordered':
        ...
        return OrderedIterator(device, action_converter, token_converter, tag_converter, config.batch_size, config.shuffle, tokens, actions_strings, tags)
    elif config.type == 'unordered':
        from app.data.iterators.unordered import UnorderedIterator
        return UnorderedIterator(device, action_converter, token_converter, tag_converter, config.batch_size, config.shuffle, tokens, actions_strings, tags)
    else:
